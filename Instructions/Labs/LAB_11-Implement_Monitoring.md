...
1. **[az104-11-vm0 \| メトリック]** ブレードの既定のグラフでは、使用可能な **メトリック名前空間** は **仮想マシン ホスト** のみです。
...
1. **[az104-11-vm0 \| メトリック]** ブレードの既定のグラフでは、この時点で **[メトリックスの名前空間]** ドロップダウン リストに、 **[仮想マシン ホスト]** エントリに加えて **ゲスト (クラシック)** エントリも含まれていることに注意してください。
...
1. **[メトリック]** ドロップダウン リストで **[Memory\\Available Bytes]** を選択し、 **[最大]** を選択して、結果のグラフを確認します。
...
1. **[モニター \| メトリック]** ブレードの **[平均 Percentage CPU 対象 az104-11-vm0]** ウィンドウで、 **[新しいアラート ルール]** （画面の右側）をクリックします。
...
1. **[アラート ルールの作成]** ブレードの **[条件]** セクションで、既存のエントリをクリックします。
...
1. **[アクション グループの作成]** ブレードの **[通知]** タブにある **[通知の種類]** ボックスの一覧で **[電子メール/SMS メッセージ/プッシュ/音声]** を選択します。 **名前** テキスト ボックスに **管理者の電子メール** と入力します。 **[詳細の編集]** アイコン (鉛筆) をクリックします。
...
1. **[確認と作成]** をクリックし、 **[確認と作成]** タブで **[作成]** をクリックします。
...
1. メッセージが表示されたら、 **student** のユーザー名とパラメーター ファイルのパスワード( Pa55w.rd1234 )を使用してサインインします。
